...
APP = celery.Celery('task', broker = BROKER_URL, backend = BROKER_URL)
...
def save_to_db(result, pname, pid, result_path_full='.'):
    ...
    con = psycopg2.connect(dbname = 'renette', user = 'renette', password = 'renette', host = os.environ['PGSQL_PORT_5432_TCP_ADDR'], port = os.environ['PGSQL_PORT_5432_TCP_PORT'])
    cursor = con.cursor()
